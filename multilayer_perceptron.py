...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self._h_bias = np.broadcast_to(1, (1, self.n_hidden))
        self._o_weights = np.random.rand(self.n_hidden, self._y.shape[1])
        self._o_bias = np.broadcast_to(1, (1, self._y.shape[1]))
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iterations):
            ...
            forward = np.dot(X, self._h_weights) + self._h_bias
            forward_af = self.hidden_activation(forward)
            forward_1 = np.dot(forward_af, self._o_weights) + self._o_bias
            forward_1_af = self._output_activation(forward_1)

            if i % 20 == 0:
                self._loss_history.append(cross_entropy(self._y, forward_1_af))

            # Error for the output layer
            error = self._y - forward_1_af

            # Delta for the output layer
            delta_o = error * self._output_activation(forward_1_af, derivative=True)

            # Calculating the error for the hidden layer by chain rule
            error = np.dot(delta_o, self._o_weights.T)

            # Delta for the hidden layer
            delta_h = error * self.hidden_activation(forward_af, derivative=True)

            # Updating the weights -
            self._o_weights = self._o_weights + self.learning_rate * np.dot(forward_af.T, delta_o)
            self._h_weights = self._h_weights + self.learning_rate * np.dot(X.T, delta_h)
    ...
    def predict(self, X):
        ...
        forward_1 = np.dot(forward_af, self._o_weights) + self._o_bias
        forward_1_af = self._output_activation(forward_1)
        forward_1_af = np.array(forward_1_af)

        # Using the np.argmax, I return the index of the max value in the row, which translates to the same index as in
        # encoded y label
        return np.argmax(forward_1_af, 1)
